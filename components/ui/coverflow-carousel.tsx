...
export function CoverflowCarousel({
  ...
  const activeDesc = active?.keterangan || active?.description;
    ...
    >
      <div className="relative">
        <div
          ...
          className="cursor-grab overflow-hidden py-10 outline-none ring-purple-400 focus-visible:ring-2 active:cursor-grabbing"
          style={{
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-[#111622] shadow-2xl will-change-transform border border-white/10",
                  cardClassName,
                )}
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-slate-800/80 p-2.5 text-white shadow-lg backdrop-blur transition hover:bg-slate-700 border border-white/20"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-slate-800/80 p-2.5 text-white shadow-lg backdrop-blur transition hover:bg-slate-700 border border-white/20"
            >
      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {activeDesc && (
            <p className="mt-2 text-[13px] text-slate-300 max-w-md leading-relaxed">
              {activeDesc}
            </p>
          )}
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-6 w-full max-w-[340px] text-[12px] bg-[#111622]/90 border border-white/10 p-3.5 rounded-xl shadow-md">
              {active.meta.map((row) => (
          ...
          {slides.map((_, index) => (
            <button
              ...
              className={cn(
                ...
                index === selected ? "opacity-100 w-6" : "opacity-30",
              )}
            />
          ))}
        </div>
